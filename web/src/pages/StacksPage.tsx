...
export function StackDetailPage() {
  ...
  const deploy = async (): Promise<void> => {
    ...
    const ok = await confirm({ title: t('stacks.deploy'), description: t('stacks.deployConfirm', { name: stack?.name }), confirmLabel: t('stacks.deploy') });
    if (!ok) return;
    try {
      await mut.deploy.mutateAsync(id);
      toast.success(t('stacks.deployed'));
    } catch (err) {
      toast.error(err instanceof ApiError ? err.message : t('common.error'));
    }
  };
  ...
  const down = async (): Promise<void> => {
    ...
    if (!ok) return;
    try {
      await mut.down.mutateAsync(id);
      toast.success(t('stacks.broughtDown'));
    } catch (err) {
      toast.error(err instanceof ApiError ? err.message : t('common.error'));
    }
  };
        ...
        {isAdmin && (
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button variant="primary" size="sm" onClick={() => void deploy()} loading={mut.deploy.isPending}>
              <Play className="h-4 w-4" /> {t('stacks.redeploy')}
            </Button>
          </div>
        )}
      </div>
